...
Beliefs can also interpret signals from sensors or existing beliefs. Signals contain data circulating in the brain.

```csharp
public class AI
{
    // Senses a transform, extracts a vector
...
{
    // Moved position to body
    public Belief<Vector3> Position { get; } = new();

    AI brain;

    // Add brain reference
    void Awake() => brain = new AI(this);
}
```
...
void FixedUpdate()
{
    var targetDirection = (brain.TargetPosition.Value - Position.Value).normalized;

    // Move toward target
    controller.Move(movementSpeed * targetDirection);
}
```
